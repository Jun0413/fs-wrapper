...
const oneFileFuncWrapper = (func, policy, mode) => (f, ...args) => {
    if (typeof f !== 'number') { checkAccessibility(policy, f, mode); }
    func(f, ...args);
};

// p1 and p2 are simply paths
const twoPathsFuncWrapper = (func, policy, _) => (p1, p2, ...args) => {
...
const openFuncWrapper = (func, policy, _) => (p, flgs? : string, ...args) => {
    let mode = 'r';
